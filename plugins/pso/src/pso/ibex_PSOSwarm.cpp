...
namespace ibex{
	...
	PSOSwarm::PSOSwarm(TreeCellOpt* tree, System* orig_sys, double c1, double c2, int nParticles, int limit) :
			tree(tree), c1(c1), c2(c2), nParticles(nParticles), orig_sys(orig_sys), limit(limit),
	...
	void PSOSwarm::resetPSO(double loup){
		...
		for(int i=0; i<nParticles; i++){
			if(!particlesArray[i]) particlesArray[i] = new PSOParticle(tree, orig_sys, c1, c2);
	...
	void PSOSwarm::executePSO(double loup){
		...
		do{
			...
			for(int i=0; i<nParticles; i++){
				...
				particlesArray[i]->updateVelocityAndPosition(tree,orig_sys,gBest,c1,c2);

...
namespace ibex {
  ...
  //  cout << i1 << "," << i2 << endl;
    ...
    if(particles[i1]->fitness(current_ub) < particles[i2]->fitness(current_ub)){
      p1=particles[i2];
      p2=particles[i1];
    }
    ...
    if(tree->search(h)){
      ...
      if (p.fitness(current_ub) > p2->fitness(current_ub)){
         *p2=p;
    //   cout << -p2->fitness(current_ub) << endl;
       }
    }

    Cell* aux;
    if (p1->fitness(current_ub) > p2->fitness(current_ub))
       aux=tree->search(p1->position);
    else
       aux=tree->search(p2->position);

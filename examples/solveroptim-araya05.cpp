...
int main(int argc, char** argv){
	...
	if (argc<11) {
		cerr << "usage: solveroptim-araya05 filename (solver|optim) filtering linear_relaxation bisection prec goal_prec timelimit taumohc nseeds (ampl|no) alpha step"  << endl;
		exit(1);
	}
	...
	//params IDA
	double alpha = atof(argv[12]); //[0,1]
	double step = atof(argv[13]);  //[-1,1]
    //***********
    ...
    if(argc>15) obj_init_bound=atof(argv[14]);
	double eq_eps= 1.e-8;
   ...
   for(int i=0; i<nseeds;i++){
	...
	if(type=="optim") o=new Optimizer(*orig_sys,*ctcxn,*bs,prec,goalprec,goalprec,samplesize,Optimizer::default_equ_eps, false);
	...
	if (o){
		o->alpha=alpha;
		o->step=step;
		 o->optimize(orig_sys->box,obj_init_bound);
	 }
    ...
    if(type=="optim") cout << argv[1] << " " << uplo << "," << loup << " " << double(total_time)/double(nseeds) << " " <<
             double(total_boxes)/double(nseeds) << " " << touts << endl;
    else if(type=="solver") cout << argv[1] << " " << nsols  << " " << double(total_time)/double(nseeds) << " " <<
             double(total_boxes)/double(nseeds) << " " << touts << endl;

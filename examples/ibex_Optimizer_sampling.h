...
#ifndef __IBEX_OPTIMIZERS_H__
#define __IBEX_OPTIMIZERS_H__
...
public:
	 ...
	 *
	 */
	Optimizer_sampling(int n, Ctc& ctc, Bsc& bsc, LoupFinder& finder, CellBufferOptim& buffer,
			int goal_var,
	 ...
	 */
	Optimizer_sampling(OptimizerConfig& config);

	/**
	 * \brief Delete *this.
	 */
	virtual ~Optimizer_sampling();
...
private:
	...
	Optimizer_sampling(const Optimizer&); // forbidden
...
inline Optimizer_sampling::Status Optimizer_sampling::get_status() const { return status; }

inline double Optimizer_sampling::get_uplo() const { return uplo; }

inline double Optimizer_sampling::get_loup() const { return loup; }

inline const IntervalVector& Optimizer_sampling::get_loup_point() const { return loup_point; }

inline double Optimizer_sampling::get_time() const { return time; }

inline size_t Optimizer_sampling::get_nb_cells() const { return nb_cells; }

inline double Optimizer_sampling::get_obj_rel_prec() const {
...
inline double Optimizer_sampling::get_obj_abs_prec() const {
	return loup-uplo;
}

inline const CovOptimData& Optimizer_sampling::get_data() const {

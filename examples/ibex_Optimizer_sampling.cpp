...
bool Optimizer_sampling::diving=false;

/*
 * TODO: redundant with ExtendedSystem.
 */
void Optimizer_sampling::write_ext_box(const IntervalVector& box, IntervalVector& ext_box) {
	int i2=0;
...
void Optimizer_sampling::read_ext_box(const IntervalVector& ext_box, IntervalVector& box) {
	int i2=0;
	for (int i=0; i<n; i++,i2++) {
		if (i2==goal_var) i2++; // skip goal variable
		box[i]=ext_box[i2];
...
Optimizer_sampling::Optimizer_sampling(int n, Ctc& ctc, Bsc& bsc, LoupFinder& finder,
...
Optimizer_sampling::Optimizer_sampling(OptimizerConfig& config) :
		n           (config.nb_var()),
...
bool Optimizer_sampling::update_loup(const IntervalVector& box, BoxProperties& prop) {
...
void Optimizer_sampling::update_uplo() {
...
void Optimizer_sampling::update_uplo_of_epsboxes(double ymin) {
...
void Optimizer_sampling::handle_cell(Cell& c) {
...
void Optimizer_sampling::contract_and_bound(Cell& c) {
...
Optimizer_sampling::Status Optimizer_sampling::optimize(const IntervalVector& init_box, double obj_init_bound) {
	start(init_box, obj_init_bound);
	return optimize();
}


Optimizer_sampling::Status Optimizer_sampling::optimize(const CovOptimData& data, double obj_init_bound) {
	start(data, obj_init_bound);
	return optimize();
}

Optimizer_sampling::Status Optimizer_sampling::optimize(const char* cov_file, double obj_init_bound) {
	CovOptimData data(cov_file);
	start(data, obj_init_bound);
	return optimize();
}

void Optimizer_sampling::start(const IntervalVector& init_box, double obj_init_bound) {
...
void Optimizer_sampling::start(const CovOptimData& data, double obj_init_bound) {
...
Optimizer_sampling::Status Optimizer_sampling::optimize() {
	Timer timer;
	timer.start();

	update_uplo();

	IntervalVector last_box=IntervalVector(n+1);
	last_box.set_empty();
	int count = 0;
	...
	try {
	     while (!buffer.empty()) {
			...
			if(last_box.is_superset(c->box)) {
				//cout << "Diving" << endl;
				minLB = false;
				count ++;
			}else{
				if(count >0){
					if (Optimizer_sampling::diving) {
						//cout << "Finishing diving" << endl;
						break;
					}else{
						cout << "final UB:" << loup << endl;
						cout << "UB point:" << loup_point.mid() << endl;
						cout << count << endl;
						loup = POS_INFINITY;
				...
				cout << "MinLB" << endl;
				cout << "lb_box:" << c->box.lb() << endl;
				cout <<  "ub_box:" << c->box.ub() << endl;
				minLB = true;
				count=1;
	...
	if (Optimizer_sampling::diving ==false){
		while (!buffer.empty()) {
...
void Optimizer_sampling::report() {

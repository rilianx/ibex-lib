	...
	for mintime in config.mintimes:
		...
		for i in range(len(config.instances)):
			...
			if times.min()<config.maxtime and times.max()>mintime:
				for id_algo in range(len(config.algos)):
					for id_algo2 in range(id_algo+1,len(config.algos)):
						if (times[id_algo]<config.maxtime or times[id_algo2]<config.maxtime) and (times[id_algo]>mintime or times[id_algo2]>mintime):
							j=0
							for factor in config.factors:
								if times[id_algo]/(times[id_algo2]+0.01) < factor:
									if id_algo==0: nb_inst[id_algo2][j]+=1;
								if times[id_algo2]/(times[id_algo]+0.01) < factor:
									if id_algo==0: nb_inst2[id_algo2][j]+=1;
								
								if times[id_algo]/(times[id_algo2]+0.01) > config.sig_gain or times[id_algo2]/(times[id_algo]+0.01) > config.sig_gain:
									instances.add(i);					
